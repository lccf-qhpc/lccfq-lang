...
def disconnected_spec():
    return {
        "type": "linear",
        "qubits": [0, 1, 2],  # include 2
        "exclusions": [],
        "connections": [(0, 1)],  # 2 is disconnected
    }


def test_valid_linear_topology(linear_spec):
    topo = QPUTopology(linear_spec)
...
def test_disconnected_topology_graph_structure(disconnected_spec):
    topo = QPUTopology(disconnected_spec)

    for q in disconnected_spec["qubits"]:
